...
def load_era5_data(grib_file=None):
    ...
    # Load GRIB file
    # ERA5 GRIB files may have multiple variables, we need wind and temperature
    ds = xr.open_dataset(
        grib_file,
        engine='cfgrib',
        backend_kwargs={
...
def convert_grib_to_csv(grib_file, output_csv, sample_points=None):
    """
    Convert GRIB file to CSV for faster loading.
    ...
    if sample_points:
        ...
        for lat, lon in sample_points:
            print(f"  Extracting ({lat}, {lon})...")

            # Find nearest grid point
            lat_idx = np.argmin(np.abs(ds.latitude.values - lat))
            lon_idx = np.argmin(np.abs(ds.longitude.values - lon))

            ds_point = ds.isel(latitude=lat_idx, longitude=lon_idx)

            # Get all times
            for time_idx in range(len(ds_point.time)):
                ds_time = ds_point.isel(time=time_idx)

                # Extract values
                u10 = float(ds_time['u10'].values if 'u10' in ds_time else ds_time['10u'].values)
                v10 = float(ds_time['v10'].values if 'v10' in ds_time else ds_time['10v'].values)

                wind_speed, wind_direction = calculate_wind_speed_direction(u10, v10)

                temp_2m = float(ds_time['t2m'].values if 't2m' in ds_time else ds_time['2t'].values)
                pressure = float(ds_time['sp'].values if 'sp' in ds_time else 0)
                ...
                record = {
                    'timestamp': pd.to_datetime(ds_time.time.values),
        ...
        # Extract all grid points (WARNING: This can be very large!)
        print("WARNING: Extracting all grid points - this may create a huge CSV!")
        # Implementation for full grid extraction would go here
